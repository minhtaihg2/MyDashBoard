...
var dberror = function (text, log, err, callback, done) {
...
var DXLogin = {
    ...
    authenticate: function (params, callback, sessionID, request, response) {
        ...
        dns.reverse(ip, function (err, domains) {
            ...
            var where = " WHERE email = '" + email + "'  AND password = '" + password + "' and ativo and emailconfirmacao";
            sql += where;
            ...
            pg.connect(global.App.connection, function (err, client, done) {
                ...
                client.query(sql, function (err, resultSelect) {
                    ...
                    if (resultSelect.rows.length == 0)
                        return dberror('Invalid user or password', `Login attempt with ${email} and ${password}`, {}, callback, done);
                    ...
                    client.query(sql, values, function (err, resultInsert) {
                        if (err)
                            return dberror('Database error', `${err.toString()} SQL: ${sql} Values: ${values.toString()}`, err, callback, done);
