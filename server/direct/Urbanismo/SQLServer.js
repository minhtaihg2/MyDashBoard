...
var SQLServer = {
    ...
    readTipoproc: function (params, callback) {
        ...
        conn.connect(function (err) {
            if (err) {
                console.log(err);
                return this.dberror('Database error', 'Connection error', err, callback);
            }
            var sql = "SELECT * FROM (";
            ...
            req.query(sql, function (err, recordset) {
                if (err) {
                    console.log(err);
                    return this.dberror('Database error', `${err.toString()} SQL: ${sql}`, err, callback);
                }
                else {
    ...
    readContrib: function (params, callback) {
        ...
        conn.connect(function (err) {
            ...
            var sql = "SELECT * FROM (";
            sql += "SELECT *, ROW_NUMBER() OVER (ORDER BY nome) as row FROM dbo.contrib WHERE nome IS NOT NULL ";
            sql += ") a WHERE row < 3";
            req.query(sql, function (err, recordset) {
                if (err) {
                    console.log(err);
...
//    var sql = "SELECT * FROM (";
//sql += "SELECT *, ROW_NUMBER() OVER (ORDER BY n_auto_embargo) as row FROM dbo.embargos ";
//sql += ") a WHERE row > 15 and row <= 20";
    ...
    readEmbargos: function (params, callback) {
        ...
        conn.connect(function (err) {
            ...
            var sql = "SELECT * FROM (";
            sql += "SELECT *, ROW_NUMBER() OVER (" + order + ") as row FROM dbo.embargos " + where;
            sql += ") a ";
            sql += paging;
            req.query(sql, function (err, recordset) {
                if (err) {
                    ...
                    var totalQuery = 'SELECT count(*) as totals FROM dbo.embargos ' + where;
                    req.query(totalQuery, function (err, recordsettotals) {
                        if (err) {
                            console.log(err);
                            return this.dberror('Database error', `${err.toString()} SQL: ${totalQuery}`, err, callback);
                        }
                        else {
                            callback(null, {
                                data: recordset,
